...
/**
  ...
  */   
    ...
    **/  
    public static function cdn($cdn,$version="",$admin=FALSE){
      $cdns=array(
        ...
        "Trumbowyg" => [
            "src" => "https://cdnjs.cloudflare.com/ajax/libs/Trumbowyg/2.13.0/trumbowyg.min.js",
            "latest" => "2.13.0",
            "css" => "https://cdnjs.cloudflare.com/ajax/libs/Trumbowyg/2.13.0/ui/trumbowyg.min.css"
          ]
        );   
  ...
  public static function isSocial(){
    ...
    $list = array("facebookexternalhit","Twitturls","Facebot","LinkedInBot","Twitterbot");
    foreach($list as $bot){
      if(strpos($ua,$bot)!==false) return true;
    }
  ...
  public static function send(array $array){    
    ...
    if(!empty(self::$config["logo"])){
      $content = str_replace("[logo]","<img src='".self::$config["url"]."/content/".self::$config["logo"]."'>",$content);
    }else{
