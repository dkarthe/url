...
class App{
	...
	protected $actions = ["user","page","contact","server","pricing","upgrade" ,"analytic","profile","ipn", "q", "jShortener", "webhook","api"];	
	/**
	 * User Variables
	 * @since 4.0
	 **/
	...
	public function run(){
		...
		if(file_exists(ROOT."/themes/{$this->config["theme"]}/functions.php")){
	...
	public function check(){
		if($info=Main::user()){
			...
			if($user=$this->db->get("user",array("id"=>"?","auth_key"=>"?"),array("limit"=>1),array($info[0],$info[1]))){
				...
				if(!is_null($this->user->planid) && $plan = $this->db->get("plans", ["id" => $this->user->planid], ["limit" => "1"])){
					$this->user->plan = $plan;
	...
	public function checkDNS($redirect = FALSE){
		...
		if($cU["host"] != $cH["host"]) return $this->_DNS404();
	}
	/**
	 * Returns User info
	 * @since 4.2
	...
	protected function user(){
		...
		if($this->logged()){
			$action = array("edit","delete","archive","expired","bundles","splash","settings","logout","verify","search","server","export","overlay","pixels", "tools", "cancel","terminate","membership","domain");
		}else{
	...
	protected function upgrade(){
		...
		if(isset($_POST["token"])){
			if(!Main::validate_csrf_token($_POST["token"])) {
				return Main::redirect(Main::href("checkout/{$this->do}/{$this->id}","",FALSE),array("danger",e("Something went wrong, please try again.")));
			}		
	...
	*/
	 ...
	 * @return  [type] [description]
	 */
	protected function _DNS404(){
		// Set Meta Tags
		Main::set("title",e("Great! Your domain name is working!"));
		Main::set("body_class","light");
		$content ="<section class='blue rounded wshadow'><h1>Yes</h1>
							<h4>Your domain name is working. Add it your account now.</h4></section>";			
		...
		return;
			...
			protected function shortener_option($form=FALSE){
				...
				if($this->config["multiple_domains"]){
					...
					if($this->pro() && $domains = $this->db->get("domains", ["userid" => $this->userid, "status" => "1"])){
	...
	protected function user_menu($option=array()){
		$menu='<ul class="nav nav-sidebar">';
			...
			$menu.='<li'.($this->permission("splash") === FALSE ? ' class="locked"': '').'><a href="'.Main::href("user/splash").'"><span class="glyphicon glyphicon-transfer"></span> '.e('Splash Pages').''.(!$this->pro() ? '<span class="label label-secondary pull-right">'.e('Pro').'</span>': '').'</a></li>';

			$menu.='<li'.($this->permission("overlay") === FALSE ? ' class="locked"': '').'><a href="'.Main::href("user/overlay").'"><span class="glyphicon glyphicon-record"></span> '.e('Overlay Pages').''.(!$this->pro() ? '<span class="label label-secondary pull-right">'.e('Pro').'</span>': '').'</a></li>';

			$menu.='<li'.($this->permission("pixels") === FALSE ? ' class="locked"': '').'><a href="'.Main::href("user/pixels").'"><span class="glyphicon glyphicon-screenshot"></span> '.e('Tracking Pixels').''.(!$this->pro() ? '<span class="label label-secondary pull-right">'.e('Pro').'</span>': '').'</a></li>';	

			$menu.='<li'.($this->permission("domain") === FALSE ? ' class="locked"': '').'><a href="'.Main::href("user/domain").'"><span class="glyphicon glyphicon-globe"></span> '.e('Custom Domain').''.(!$this->pro() ? '<span class="label label-secondary pull-right">'.e('Pro').'</span>': '').'</a></li>';		
			...
			if($this->config["pro"]){
				$menu.='<li><a href="'.Main::href("user/membership").'"><span class="glyphicon glyphicon-credit-card"></span> '.e('Membership').'</a></li>';
			}
		...
		protected function widget_top_urls($option=array()){
      	...
      	if(empty($data)){
        	...
        	$html.="<ul>";
        		foreach ($data as $url) {
        		  $html.="<li>
        		  <a href='{$this->config["url"]}/{$url->alias}{$url->custom}+' target='_blank'>
        		  &nbsp;<img src='https://www.google.com/s2/favicons?domain={$url->url}' alt='favicon'>
        		  ".(empty($url->meta_title)?"{$this->user->domain}/{$url->alias}{$url->custom}":Main::truncate(fixTitle($url->meta_title),30))."
  	...
  	protected function widget_export($id = ""){
  		if($this->permission("export") === FALSE){
				$html='<div class="panel panel-default panel-body" id="'.__FUNCTION__.'">';
						...
						$html.="<a class='btn btn-secondary' href='".Main::href("pricing")."' rel='nofollow'>".e("Upgrade")."</a>";							
				$html.='</div>';  			
				return $html;
  		}
			$html='<div class="panel panel-default panel-body" id="'.__FUNCTION__.'">';
  ...
  protected function count($count,$option=""){
		$system=array("urls","users","clicks","user_urls","user_bundles","user_clicks","user_bundle_urls","user_public_urls","user_public_bundles","user_public_bundle_urls","user_splash","user_pixels","user_overlay","user_domain");
		$fn = "count_$count";
		...
		return FALSE;
  		...
  		protected function count_user_splash(){
  			return $this->db->count("splash","userid='{$this->userid}'");
  		}  		
  		/**
  		...
  		protected function count_user_overlay(){
  			return $this->db->count("overlay","userid='{$this->userid}'");
  		} 
  		/**
  		...
  		protected function count_user_domain(){
  			return $this->db->count("domains","userid='{$this->userid}'");
  		}   		
  		/**
  		...
  		protected function count_user_pixels(){
  			...
  			if(!$fb = is_array($this->user->fbpixel) ? $this->user->fbpixel : json_decode($this->user->fbpixel)){
					$fb = [];
  			}
				if(!$adw = is_array($this->user->adwordspixel) ? $this->user->adwordspixel : json_decode($this->user->adwordspixel)){
					$adw = [];
				}
				if(!$lkd = is_array($this->user->linkedinpixel) ? $this->user->linkedinpixel : json_decode($this->user->linkedinpixel)){
					$lkd = [];
				}
				if(!$tw = is_array($this->user->twitterpixel) ? $this->user->twitterpixel : json_decode($this->user->twitterpixel)){
					$tw = [];
				}
				if(!$adr = is_array($this->user->adrollpixel) ? $this->user->adrollpixel : json_decode($this->user->adrollpixel)){
					$adr = [];
				}

				return count($fb) + count($adw) + count($lkd) + count($tw) + count($adr);		
  		}   	  		
  /**
	...
	public function ads($size,$text=TRUE,$breadcrumb=""){
		...
		if(in_array($size, array("728","468","300","resp","splash","frame"))){			
			...
			if($this->config["ads"]){
				...
				return "<div class='ads ads-".($size+1)." clearfix'>".($text?"<p class='text'><small class='pull-left'>".e('Advertisment')."</small><a href='{$this->config["url"]}/upgrade' class='pull-right'><small>(".e("Remove Ads").")</small></a></p>":"")."{$ad->code}</div>";				
			}
		}		
		return;		
	}
