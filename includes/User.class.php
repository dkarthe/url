  ...
  private function edit(){
  	...
  	if(empty($this->id) || !is_numeric($this->id)) return Main::redirect("user",array("danger",e("This URL doesn't exist.")));
		...
		if($this->config["multiple_domains"]){
			...
			if(!empty($this->user->domain)){
				$domain_list .= '<option value="'.strtolower($this->user->domain).'">'.ucfirst(str_replace("https://","",str_replace("http://", "",$this->user->domain))).'</option>';
			}					
   ...
   */
	...
	protected function pixels(){
  	...
  	if($this->permission("pixels") === FALSE){
		...
		if($fb = json_decode($this->user->fbpixel)){
			$this->user->fbpixel = $fb;
		}
		if($adw = json_decode($this->user->adwordspixel)){
			$this->user->adwordspixel = $adw;
		}
		if($lkd = json_decode($this->user->linkedinpixel)){
			$this->user->linkedinpixel = $lkd;
		}
		if($tw = json_decode($this->user->twitterpixel)){
			$this->user->twitterpixel = $tw;
		}		
		if($adr = json_decode($this->user->adrollpixel)){
			$this->user->adrollpixel = $adr;
		}	

		$total = $this->count("count_user_pixels");		
