...
class Short extends App{	
    ...
    // If require registration
		...
		if($this->logged() && isset($array["domain"]) && $this->db->get("domains", ["userid" => $this->userid, "domain" => $array["domain"], "status" => "1"])){
		...
		if($this->userid && (!isset($array["password"]) || empty($array["password"])) &&  (!isset($array["custom"]) || empty($array["custom"]))){
			if($data = $this->db->get("url",array("url"=>"?","userid"=>"?","location"=>"?","pass"=>"?", "devices" => "?", "pixels" => "?", "type" => "?"), array("limit"=>"1"), array($url, $this->userid, $countries,"", $devices, "", $array["type"]))){
		...
		if($this->logged() && $this->user->plan->numurls > 0 && $this->count_user_urls() >= $this->user->plan->numurls) return array('error' => 1, 'msg' => e('You have maxed your short URLs limit. Either delete existing URLs or upgrade to a premium plan.'));

		if($array["type"] == "frame" || preg_match("!overlay-!", $array["type"])){
		...
		if(in_array($ext, array(".zip",".rar",".7z",".flv",".mp4",".avi",".mp3",".jpeg",".png",".jpg",".gif",".mk4",".iso"))){
			...
			if(!empty($info)){
				...
				if(strlen($meta_description) > 255){
					$meta_description = Main::truncate($meta_description,250);
				}
			}			
		}
	...
	private function redirect(){
		...
		if($url->pro && preg_match("~overlay-(.*)~", $url->type) && $overlay = $this->db->get("overlay", ["id" => str_replace("overlay-", "", $url->type), "userid" => $user->id], ["limit" => 1]))	{
			return $this->overlay($url, json_decode($overlay->data));
		}	
		// Custom Splash Page
